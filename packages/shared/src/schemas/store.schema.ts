...
// ============================================================
// V3 — Bespoke Styles (AI-generated per-store CSS)
// The hybrid: structure is config-driven, aesthetics are generative
// ============================================================

export const BespokeHeroStyles = z.object({
  /** CSS custom properties scoped to the hero — generated by AI per-store */
  fontSize: z.string().optional(),          // e.g. "clamp(2.5rem, 8vw, 5.5rem)"
  lineHeight: z.string().optional(),        // e.g. "0.95"
  letterSpacing: z.string().optional(),     // e.g. "-0.04em"
  overlayGradient: z.string().optional(),   // Full CSS gradient string
  textShadow: z.string().optional(),        // e.g. "0 4px 32px rgba(0,0,0,0.3)"
  ctaStyle: z.string().optional(),          // e.g. "rounded-full" or "sharp"
  accentElement: z.string().optional(),     // e.g. "underline-brush" | "glow" | "none"
}).default({});

export const BespokeCardStyles = z.object({
  /** Per-store card personality */
  hoverTransform: z.string().optional(),    // e.g. "translateY(-6px) scale(1.01)"
  badgeGradient: z.string().optional(),     // e.g. "linear-gradient(135deg, #E94560, #FF6B6B)"
  priceColor: z.string().optional(),        // Override for price display
  shadowOnHover: z.string().optional(),     // e.g. "0 12px 40px rgba(0,0,0,0.12)"
}).default({});

export const BespokeStyles = z.object({
  hero: BespokeHeroStyles.default({}),
  card: BespokeCardStyles.default({}),
  /** Full-bleed accent section — one section gets the primary color takeover */
  accentSectionCSS: z.string().optional(),  // e.g. "background: #C2185B; color: #fff;"
  /** Signature interaction per vertical */
  signatureEffect: z.enum(['parallax-drape', 'sparkle-overlay', 'organic-reveal', 'none']).default('none'),
}).default({});
...
export const DesignTokens = z.object({
  ...
  bespokeStyles: BespokeStyles.default({}),
});
...
export const SectionConfig = z.object({
  ...
  // V3: Irregular spatial rhythm — controls gap before this section
  vibeWeight: z.number().min(0).max(2).optional(),   // 0.2=compressed, 1=normal, 1.5=expanded
  // V3: Color intensity — makes sections bold or subtle
  colorIntensity: z.enum(['low', 'medium', 'high']).optional(), // 'high' = primary bg, white text
});
